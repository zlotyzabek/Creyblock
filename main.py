...
with open(f"assest/textures/texturesBlockLoad.txt", "r") as f:
    ...
    for i, line in enumerate(f):
        ...
        if readTEMPtextures[1] == "c":
            textureBlock[i] = [texture, 'cube']
        elif readTEMPtextures[1] == "a":
            textureBlock[i] = [texture, 'quad']
    # SKY
...
class Equipment(Entity):
    def __init__(self):
        ...
        for szer in range(10):
            eqMenage[("ID", szer)] = 0
            eqMenage[("Hot Bar", szer)] = Entity(position = ((szer - 5) * 1.1 + 0.5, -4.5), model = "cube", parent = self, scale = (1, 1), texture = "assest/textures/hotBar.png")
            eqMenage[("Item", szer)] = Button(position = ((szer - 5) * 1.1 + 0.5, -4.5), texture = textureBlock[eqMenage[("ID", szer)]], color=color.white, parent=self, scale = (0.8, 0.8), on_click = self.equipmentSelect, always_on_top = True, highlight_color = color.white66, name = str(tempEquipment))
            eqMenage[("Count", szer)] = Text(position = (0.1, -0.2), scale = (10, 10),text = "",parent = eqMenage[("Item", szer)], always_on_top = True, name = str(tempEquipment))
            tempEquipment += 1


        eqMenage["Select"] = 1
        eqMenage["Big"] = Entity(parent = self, scale = (1, 1), visible = False, enabled = False)
        eqMenage["Big", "equipment"] = Entity(model = 'quad', texture = 'assest/textures/equipment.png', color=color.white, parent=eqMenage["Big"], scale = (15,8))
        eqMenage[("Big", "Select")] = Entity(model = 'quad', texture = 'assest/textures/air.png', color=color.white, parent=eqMenage["Big"], world_scale = (1,1))
        eqMenage[("Big", "Select", "Count")] = Text(position = (0.1, -0.2), scale = (10, 10),text = "",parent = eqMenage[("Big", "Select")], always_on_top = True)
        ...
        for szer in range(10, 26):
            eqMenage[("ID", szer)] = 0
            eqMenage[("Item", szer)] = Button(position = (-0.38 + (int((szer - 10) % 4) / 8), 0.3 - (int((szer - 10) / 4) / 5)), model = 'quad', texture = textureBlock[eqMenage[("ID", szer)]], color=color.white, parent=eqMenage["Big", "equipment"], scale = (0.09, 0.16), always_on_top = True, on_click = self.equipmentSelect, name = str(tempEquipment))
            eqMenage[("Count", szer)] = Text(position = (0.1, -0.2), scale = (10, 10),text = "",parent = eqMenage[("Item", szer)], always_on_top = True, name = str(tempEquipment))
            tempEquipment += 1

        for i in range(0, 26):
            try:
                eqMenage[("ID", i)] = playerInfo[3][i][0]
                eqMenage[("Item", i)].texture = playerInfo[3][i][1]
                eqMenage[("Count", i)].text = playerInfo[3][i][2]
            except Exception:
                pass
    ...
    def equipmentSelect(self):
        if eqMenage[("Big", "Select", "mode")] == 0:
            if eqMenage[("Count", int(mouse.hovered_entity.name))].text != "":
                ...
                eqMenage[("Big", "Select", "temp")] = [eqMenage[("Count", name)].text, eqMenage[("ID", name)], eqMenage[("Item", name)].texture, int(mouse.hovered_entity.name)]
...
class Voxel(Button):
    def __init__(self, position = (0,0,0), textureBlock = 0, model = 'cube', **kwargs):
        super().__init__(model = model, texture = textureBlock, color = color.gray, parent = voxels, shader = lit_with_shadows_shader, position = position)
...
mouseRatio = window.windowed_size[0] / window.screen_resolution[0]
player = Player()
voxels = Entity()
sky = Sky()
eqMenage = {}
...
def save():
    ...
    for i in range(0, 26):
        try:
            playerInfo[3][i][0] = eqMenage[("ID", i)]
            playerInfo[3][i][1] = eqMenage[("Item", i)].texture
            playerInfo[3][i][2] = eqMenage[("Count", i)].text
        except Exception:
            playerInfo[3].append([])
            playerInfo[3][i].append(eqMenage[("ID", i)])
            playerInfo[3][i].append(eqMenage[("Item", i)].texture)
            playerInfo[3][i].append(eqMenage[("Count", i)].text)
                ...
                if blocks[int(player.x) + szer, int(player.y) + wys][3] == 0:
                    for i in range(3):
                        if blocks[int(player.x) + szer, int(player.y) + wys][i] != 0:
                            Voxel((int(player.x) + szer, int(player.y) + wys, 0 + i), textureBlock[blocks[int(player.x) + szer, int(player.y) + wys][i]][0], model = textureBlock[blocks[int(player.x) + szer, int(player.y) + wys][i]][1])
