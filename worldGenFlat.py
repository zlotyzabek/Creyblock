...
if trybGeneratora == "flat":
    ...
    dlugosc = int(input("->"))
    print("Wpisz na ile kratek ma być świat(Wysokość)")
    wysokosc = int(input("->"))


    wysokosc2 = (1080 - (wysokosc * 48))
    warstwaGeneratora = 0
    f = open("save.mov", "w")
    for wy in range(0,wysokosc * 48, 48):
        if warstwaGeneratora == 0:
            for sz in range(0, dlugosc * 48, 48):
                f.write(f"{sz},{0 + wysokosc2},2\n")
                warstwaGeneratora +=1
        else:
            for sz in range(0, dlugosc * 48, 48):
                f.write(f"{sz},{wy + wysokosc2},1\n")
